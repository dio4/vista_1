...
SELECT e.client_id AS cl_id
 ...
 (
    ...
    INNER JOIN rbClientMonitoringKind cmk ON cmk.id = cm.kind_id
    WHERE (%s)
    AND cmk.name LIKE 'К' AND (cm.endDate is null OR cm.endDate >= DATE('%s'))
    GROUP BY e.client_id
    )s
  ) AS all_this_year,
  ...
  where chg2.name LIKE 'К') col13
   ...
   AND DATE(s.cm_setDate) >= DATE('%s') AND DATE(s.cm_setDate ) <= DATE('%s')
    ...
    return db.query(stmt % (query, begDate.toString('yyyy-MM-dd'), endDate.toString('yyyy-MM-dd'),
                            ...
                            query, endDate.toString('yyyy-MM-dd'),
                            query, endDate.toString('yyyy-MM-dd'),
                            query, begDate.toString('yyyy-MM-dd'), endDate.toString('yyyy-MM-dd'), begDate.toString('yyyy-MM-dd'), endDate.toString('yyyy-MM-dd')))
...
class CReportReHospitalization(CReport):
    ...
    def build(self, params):
        ...
        tableColumns = [
            ...
            ('8%', [u'Обратилось впервые в течение года за консультативно-лечебной помощью'], CReportBase.AlignLeft),
            ('8%', [u'из них с впервые в жизни установленным диагнозом'], CReportBase.AlignLeft),
            ('6%', [u''], CReportBase.AlignLeft),
            ('6%', [u''], CReportBase.AlignLeft),
            ('8%', [u'Прекратили обращаться за консультативно-лечебной помощью'], CReportBase.AlignLeft),
            ('8%', [u'из них в связи с выздоровлением или стойким улучшением'], CReportBase.AlignLeft),
            ('8%', [u' Пациенты, которым продолжает оказываться консультативно-лечебная помощь'], CReportBase.AlignLeft),
            ('6%', [u''], CReportBase.AlignLeft),
            ('6%', [u''], CReportBase.AlignLeft),
            ('15%', [u'Из числа пациентов, показанных в гр.10, переведено в течение года из группы пациентов,\
             находившихся под диспансерным наблюдением'], CReportBase.AlignLeft)
        ]
        ...
        lst_scnd_row = [u'', u'', u'', u'', u'', u'', u'из них детей:', u'',
                        u'', u'', u'из них детей:', u'', u'']
        ...
        for j in range(0, len(l)):
            ...
            #query2 = selected_date2(params, query_str)
           # query2_next = selected_date2(params, query_str)
            #query2_next.next()
            self.setQueryText(forceString(query.lastQuery()))  # this does nothing?
            #self.setQueryText(forceString(query2.lastQuery()))
            #self.setQueryText(forceString(query2_next.lastQuery()))
            #counter_col_13 = 0

           #while query2.next():
           #     query2_next.next()
            #    record = query2.record()
            #    record2 = query2_next.record()

            #    if str(forceString(record.value('client_id'))) == str(forceString(record2.value('client_id'))) \
            #            and str(forceString(record.value('eventType_id'))) == '2' \
            #            and str(forceString(record2.value('eventType_id'))) == '6':
             #       counter_col_13 += 1
            #if ii != 5:
                #table.setText(kk + 4, 12, str(counter_col_13))
           # counter_col_13 = 0
            #kk = kk + 1
            ...
            while query.next():
                ...
                if ii != 5:
                    ...
                    table.setText(ii + 4, 7, forceString(val_col_8))
    ...
    def merge_cells(self, table):
        ...
        table.mergeCells(1, 4, 2, 1)  # 7th column, 1,2 rows
        table.mergeCells(2, 4, 2, 1)
        table.mergeCells(1, 9, 2, 1)  # 7th column, 1,2 rows
        table.mergeCells(2, 9, 2, 1)
        table.mergeCells(0, 7, 3, 1)  # 8th column, 1,2 rows
        table.mergeCells(1, 7, 3, 1)
        table.mergeCells(2, 7, 3, 1)
        table.mergeCells(0, 8, 3, 1)  # 9th column, 1,2 rows
        table.mergeCells(1, 8, 3, 1)
        table.mergeCells(2, 8, 3, 1)
