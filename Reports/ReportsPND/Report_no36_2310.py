...
class CReportReHospitalization(CReport):
    ...
    def build(self, params):
        ...
        tableColumns = [
            ('18%', [u'Из общего числа поступивших больных (стр. 1, 23, 26 гр.4 т.2300):'], CReportBase.AlignLeft),
            ('18%', [u''], CReportBase.AlignLeft),
            ('18%', [u''], CReportBase.AlignLeft),
            ('18%', [u'Число лиц, недобровольно освидетельствованных стационаром (кроме поступивших '
                    u'по скорой психиатрической помощи и освидетельствованных в специализированных '
                    u'амбулаторно-поликлинических организациях)'], CReportBase.AlignLeft),
            ('15%', [u''], CReportBase.AlignLeft),
            ('14%', [u'Число пациентов, в отношении которых получено постановление судьи о '
                    u'недобровольной госпитализации (по ст. 35)'], CReportBase.AlignLeft)
        ]
        ...
        lst_scnd_row = [u'', u'', u'поступило недобровольно', u'', u'', u'']
        i = table.addRow()
        for j in range(0, len(lst_scnd_row)):
            table.setText(i, lst_count[j], lst_scnd_row[j])

        lst_thrd_row = [u'переведено из других психиатрических стационаров', u'по скорой психиатрической помощи',
                        u'по направлению специализированной амбулаторно-поликлинической организации', u'всего',
                        u'из них - госпитализировано недобровольно по ст.29', u'']
        i = table.addRow()
    ...
    def merge_cells(self, table):
        table.mergeCells(0, 0, 1, 3)  # first row, 1-3 columns
        table.mergeCells(0, 1, 1, 3)
        table.mergeCells(0, 2, 1, 3)
        table.mergeCells(0, 3, 1, 2)  # first row, 4,5 columns
        table.mergeCells(0, 4, 1, 2)
        table.mergeCells(1, 3, 1, 2)  # second row, 4,5 columns
        table.mergeCells(1, 4, 1, 2)
        table.mergeCells(1, 0, 2, 1)  # first column, 2,3 rows
        table.mergeCells(2, 0, 2, 1)
        table.mergeCells(1, 1, 1, 2)  # second row, 1,2 columns
        table.mergeCells(1, 2, 1, 2)
        table.mergeCells(0, 5, 3, 1)  # last column, 1-3rows
        table.mergeCells(1, 5, 3, 1)
        table.mergeCells(2, 5, 3, 1)
        table.mergeCells(0, 3, 2, 2)  #
        table.mergeCells(1, 3, 2, 2)
