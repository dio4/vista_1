...
class CReportReHospitalization(CReport):
    ...
    def __init__(self, parent):
        ...
        self.setTitle(u"Психиатрическая экспертиза")
    ...
    def build(self, params):
        ...
        tableColumns = [
            ('17%', [u''], CReportBase.AlignLeft),
            ('3%', [u'№ строки'], CReportBase.AlignLeft),
            ('9%', [u'Число лиц, прошедших экспертизу'], CReportBase.AlignLeft),
            ('9%', [u''], CReportBase.AlignLeft),
            ('9%', [u''], CReportBase.AlignLeft),
            ('13%', [u''], CReportBase.AlignLeft),
            ('13%', [u''], CReportBase.AlignLeft)
        ]

        table = createTable(cursor, tableColumns)
        cursor.movePosition(QtGui.QTextCursor.End)
        cursor.insertText(u'*) Полные сведения приводятся в форме №38 "Сведения о работе судебно-психиатрической комиссии"')
        query = selectData(params)
        self.setQueryText(forceString(query.lastQuery()))  # this does nothing?

        lst_naimenovanie = [u'', u'', u'', u'',
                            u'Число лиц, прошедших экспертизу - всего',
                            u"из них прошедших экспертизу амбулаторно"]
        lst_numbers = [u'', u'', u'', u'', u'1', u'2']
        lst_count = [0, 1, 2, 3, 4, 5, 6]
        lst_scnd_row = [u'', u'', u'', u'в том числе', u'', u'', u'']
        i = table.addRow()
        for j in range(0, len(lst_scnd_row)):
            table.setText(i, lst_count[j], lst_scnd_row[j])

        lst_thrd_row = [u'', u'', u'всего', u'военную', u'из них призывникам', u'трудовую', u'судебно-психиатрическую*)']
        i = table.addRow()
    ...
    def merge_cells(self, table):
        table.mergeCells(0, 0, 3, 1)  # first col, row 0,1,2
        table.mergeCells(1, 0, 3, 1)
        table.mergeCells(2, 0, 3, 1)
        table.mergeCells(0, 1, 3, 1)  # second col, row 0,1,2
        table.mergeCells(1, 1, 3, 1)
        table.mergeCells(2, 1, 3, 1)
        table.mergeCells(0, 0, 1, 5)  # first row, col 3-7
        table.mergeCells(0, 1, 1, 5)
        table.mergeCells(0, 2, 1, 5)
        table.mergeCells(0, 3, 1, 5)
        table.mergeCells(0, 4, 1, 5)
        table.mergeCells(1, 3, 1, 4)  # second row, col 4-7
