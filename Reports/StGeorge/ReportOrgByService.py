...
from Reports.StGeorge.Ui_ReportOrgByServiceSetup import Ui_ReportOrgByServiceSetupDialog
...
def selectData(params):
    ...
    group = [tableOrgStructure['id'], tableRealActionType['code']]

    order = [tableOrgStructure['id'], tableRealActionType['code']]
...
class MasterRecord:
    ...
    def currOrgName(self):
        if not self.orgs: return None
        else: return self.orgs[-1].name

    def addElement(self, orgName, service):
        if orgName == self.currOrgName(): self.orgs[-1].addService(service)
        else: self.orgs.append(OrgRecord(orgName, service))
        self.amount += service.amount
        self.sum += service.sum
...
def buildTable(table, masterRecord):
    for org in masterRecord.orgs:
        ...
        table.setText(row, 0, org.name, fontBold=True)
        ...
        row = table.addRow()
        table.setText(row, 0, u'Итого по отделению', fontBold=True)
...
class CReportOrgByService(CReport):
    def __init__(self, parent):
        CReport.__init__(self, parent)
        self.setTitle(u'Анализ нагрузки на отделение по услугам')

    def getSetupDialog(self, parent):
        return CReportOrgByServiceSetupDialog(parent=parent, title=self.title())
    ...
    def build(self, params):
        ...
        tableColumns = [
            ...
            ('20%',         [u'Сумма руб. по прейскуранту'],    CReportBase.AlignLeft)
        ]
        ...
        while query.next():
            ...
            masterRecord.addElement(orgName, service)

        buildTable(table, masterRecord)

        return doc

class CReportOrgByServiceSetupDialog(QtGui.QDialog, Ui_ReportOrgByServiceSetupDialog):
...
def main():
    ...
    CReportOrgByService(None).exec_()
