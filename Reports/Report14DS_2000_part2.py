...
def selectData(params, org_id, bedProfile, bed_id):
    ...
    FROM(
       SELECT SUM(vypisanny.detey) AS detey, SUM(vypisanny.vzroslye) AS vzroslye, SUM(vypisanny.starshe) AS starshe FROM ( # kolicestvo na pacienti
                    SELECT
                      IF (c.sex=1,
                      ((YEAR(e.setDate) - YEAR(DATE(c.birthDate)) - (RIGHT(DATE(e.setDate), 5)
...
def selectData_PacientDays(params, provedeno_dney, bedProfile, org_id, bed_id):
    stmt = u'''
      SELECT e.setDate AS e_set, e.execDate AS e_exec, et.id AS et_id, c.id AS c_id
          FROM Event e
...
class CReportF14DS(CReport):
    def __init__(self, parent):
        ...
        self.setTitle(u"Дневной стационар (продолжение)")
    ...
    def build(self, params):
        ...
        tableColumns = [
            ...
            ('7%', [u'Дневные стационары медицинских организаций, оказывающих медицинскую помощь '
                    u'в амбулаторных условиях', u'Число коек', u'для взрослых', u'на конец года'], CReportBase.AlignLeft),
        ...
        if profileBedId is None or not profileBedId:
            ...
            for i in range(0, len(profileBedId)):
                rbProfileBed = db.getRecordEx(tblrbHospitalBedProfile, cols='name, row_code', where=tblrbHospitalBedProfile['id'].eq(profileBedId[i]))
        ...
        lst_age = [
            "((YEAR(e.setDate) - YEAR(DATE(c.birthDate)) - (RIGHT(DATE(e.setDate), 5) < RIGHT(DATE(c.birthDate), 5))) >= 18)",
            "(((YEAR(e.setDate) - YEAR(DATE(c.birthDate)) - (RIGHT(DATE(e.setDate), 5) < RIGHT(DATE(c.birthDate), 5))) >= 60) AND c.sex = 1) "
            "OR (((YEAR(e.setDate) - YEAR(DATE(c.birthDate)) - (RIGHT(DATE(e.setDate), 5) < RIGHT(DATE(c.birthDate), 5))) >= 55) AND c.sex = 2)",
            "((YEAR(e.setDate) - YEAR(DATE(c.birthDate)) - (RIGHT(DATE(e.setDate), 5) < RIGHT(DATE(c.birthDate), 5))) < 18)"
            "AND ((YEAR(e.setDate) - YEAR(DATE(c.birthDate)) - (RIGHT(DATE(e.setDate), 5) < RIGHT(DATE(c.birthDate), 5))) >=0)"]
        ...
        for bed_id in range(0, len(bedProfile_dict)):
            ...
            for org_id in range(0, len(lst_id)):
                ...
                # This is done in order to make the query smaller and simpler (just change the age range by looping in for cycle)
                #
    ...
    def createRowsCols(self, table, bedProfile_dict):
        ...
        # fifth row
        #
        lst_ffth_row = [u'1', u'2', u'13', u'14', u'15', u'16', u'17', u'18', u'19', u'20', u'21', u'22']
...
def main():
    ...
    QtGui.qApp.currentOrgId = lambda:  230493 #230493 #230226  # 229005
