...
from Ui_ExportRbComplain_Wizard_1 import Ui_ExportRbComplain_Wizard_1
from Ui_ExportRbComplain_Wizard_2 import Ui_ExportRbComplain_Wizard_2
...
def ExportRbComplain(parent):
    fileName = forceString(getVal(
        QtGui.qApp.preferences.appPrefs, 'ExportRbComplainFileName', ''))
    exportAll = forceBool(getVal(
        QtGui.qApp.preferences.appPrefs, 'ExportRbComplainExportAll', 'False'))
    compressRAR = forceBool(getVal(
        QtGui.qApp.preferences.appPrefs, 'ExportRbComplainCompressRAR', 'False'))
    dlg = CExportRbComplain(fileName, exportAll,  compressRAR,  parent)
    dlg.exec_()
    QtGui.qApp.preferences.appPrefs['ExportRbComplainFileName'] = toVariant(
                                                                            dlg.fileName)
    QtGui.qApp.preferences.appPrefs['ExportRbComplainExportAll'] = toVariant(
                                                                            dlg.exportAll)
    QtGui.qApp.preferences.appPrefs['ExportRbComplainCompressRAR'] = toVariant(
                                                                            dlg.compressRAR)
...
class CExportRbComplainWizardPage1(QtGui.QWizardPage, Ui_ExportRbComplain_Wizard_1, CConstructHelperMixin):
    ...
    def preSetupUi(self):
        self.addModels('Tree', CDBTreeModel(self, 'rbComplain', 'id', 'group_id', 'name', order='code'))
...
class CExportRbComplainWizardPage2(QtGui.QWizardPage, Ui_ExportRbComplain_Wizard_2):
    def __init__(self,  parent):
        self.done = False
        QtGui.QWizardPage.__init__(self, parent)
        self.setupUi(self)
    ...
    def on_btnExport_clicked(self):
        ...
        if not self.parent.exportAll:
            ...
            if idList == []:
                QtGui.QMessageBox.warning(self, u'Экспорт справочника "Жалобы"',
...
class CExportRbComplain(QtGui.QWizard):
    def __init__(self, fileName,  exportAll, compressRAR,  parent=None):
        QtGui.QWizard.__init__(self, parent, Qt.Dialog)
        self.setWizardStyle(QtGui.QWizard.ModernStyle)
        self.setWindowTitle(u'Экспорт справочника "Жалобы"')
        self.selectedItems = {}
        self.fileName= fileName
        self.exportAll = exportAll
        self.compressRAR = compressRAR
        self.addPage(CExportRbComplainWizardPage1(self))
        self.addPage(CExportRbComplainWizardPage2(self))
