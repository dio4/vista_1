...
from library.Utils import forceDate, forceDouble, forceInt, forceRef, forceString, forceStringEx, \
    ...
    getEventProfileForService

getcontext().prec = 12
def exportR23NativeS(widget, accountId, accountItemIdList):
...
class CExportPage1(CBaseExportPage1):
    ...
    def createQuery(self):
        ...
        # FIXME: pirozhok: запрос необходимо разбить на части, а также следует исключить костыли, связанные с отбором
        # уникальных записей /* @see: self.exportListP, self.exportedClients */
    ...
    def process(self, dbf, record, bookkeeperCode, accNumber, settleDate, accType, payerOGRN):
        ...
        outgoingRefNumber = forceInt(record.value('outgoingRefNumber'))
        ...
        isFromRelegateOrg = (setOrgCode != 0)
        ...
        if not ((sn, modern) in self.exportedClients):
            ...
            # calcAgeInDays(birthDate, begDate) < 30
            ...
            # FIXME: pirozhok, если правильно понимаю, то необходимо смотреть на таблицу Referral на поле isSend вместо
            # FIXME: инфа получаемой из Event.relegateOrg_id. В общем необходимо снести костыли и сделать все на подобии
            # FIXME: формата R23Native
            if isFromRelegateOrg and forceString(referralNumber) and forceString(referralNumber) != ' ':
            ...
            dbfRecord['MP'] = '8' if forceInt(record.value('order')) in [5, 8] else '2' if forceInt(record.value('order')) == 2 else '1'
            ...
            """
            # if forceDate(record.value('receiveDirectionDate')) != QtCore.QDate():
            #    dbfRecord['NAPR_D'] = pyDate(forceDate(record.value('receiveDirectionDate')))
            # elif forceDate(record.value('referralDate')) != QtCore.QDate():
        ...
        # daysFact = forceInt(record.value('amount'))
        ...
        # дата начала выполнения услуги обязательное
        dbfRecord['DATN'] = pyDate(forceDate(record.value('servBegDate')))#pyDate(begDate)
        if self.ignoredCsg is None:
            from Accounting.Utils import getIgnoredCsgList
            self.ignoredCsg = getIgnoredCsgList()
        ...
        # способ оплаты медицинской помощи SPR17 (DROPED i4432)
        # dbfRecord['KSO']    = forceString(record.value('medicalAidUnitFederalCode'))
