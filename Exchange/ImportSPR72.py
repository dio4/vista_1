...
def ImportSPR72(widget):
    dlg = CImportSPR72(widget)
    dlg.exec_()


class CImportSPR72(CDBFimport, QtGui.QDialog, Ui_Dialog):
    def __init__(self, parent):
        QtGui.QDialog.__init__(self, parent)
        self.setupUi(self)
        super(CImportSPR72, self).__init__(self, self.log)
        self.progressBar.setFormat('%v')
        self.edtFileName.setText(forceString(getVal(QtGui.qApp.preferences.appPrefs, 'ImportSPR72FileName', '')))
        self.setWindowTitle(u'Загрузка услуг из SPR72')
    ...
    @staticmethod
    def fillSPR72Record(db, table, record, begDate=None, endDate=None, CODE=None):
    ...
    def startImport(self):
        ...
        for row in dbf:
            ...
            SPR72List = db.getRecordList(tblSPR72,
                                         cols='*',
                                         where=db.joinAnd([
                                             tblSPR72['begDate'].eq(begDate),
                                             tblSPR72['endDate'].eq(endDate),
                                             tblSPR72['CODE'].eq(code)]))
            if len(SPR72List) > 1: self.addDuplicateAlertToLog(u'SPR72 (mes.SPR72)', code, [r.value('id') for r in SPR72List])
                ...
                SPR72List = db.getRecordList(tblSPR72,
                                            ...
                                            where=db.joinAnd([
                                                tblSPR72['begDate'].eq(begDate),
                                                tblSPR72['endDate'].eq(dateRightInfinity),
                                                tblSPR72['CODE'].eq(code)]))
                if len(SPR72List) > 1:
                    self.addDuplicateAlertToLog(u'SPR72 (mes.SPR72)', code, [r.value('id') for r in SPR72List])
                    for record in SPR72List: self.fillSPR72Record(db, tblSPR72, record, endDate=endDate)
                elif len(SPR72List) == 1: self.fillSPR72Record(db, tblSPR72, SPR72List[0], endDate=endDate)
                else: self.fillSPR72Record(db, tblSPR72, tblSPR72.newRecord(), begDate, endDate, code)

            nprocessed += 1
        self.log.append(u'Обработано %d записей' % nprocessed)
        self.log.append(u'готово')
        QtGui.qApp.preferences.appPrefs['ImportSPR72FileName'] = toVariant(self.edtFileName.text())
