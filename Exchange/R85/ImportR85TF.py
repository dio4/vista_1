...
from Events.Utils import CFinanceType, getEventLengthDays
...
from library.Utils import getPref, getClassName, setPref, forceRef, forceDate, forceStringEx, toVariant, forceDouble, \
    forceInt, unformatSNILS, CLogHandler, forceTr, calcAge, MKBwithoutSubclassification, forceBool, calcAgeInDays, \
    calcAgeInYears
...
from Exchange.R85.ImportR85DDTF import CImportR85DDTFEngine

def importR85TF(widget, orgId):
...
class CImportR85TFEngine(CExchangeImportR85Engine):
    ...
    def __init__(self, db, orgId, progressInformer = None, format=None):
        super(CImportR85TFEngine, self).__init__(progressInformer)
    ...
    def getServiceInfo(self, code):
        if not self._serviceInfoCache.has_key(code):
            r = self._db.getRecordEx('rbService',
                                     u'id, name, adultUetDoctor',
                                     u'code = \'%s\'' % code)
            if r: self._serviceInfoCache[code] = (forceRef(r.value('id')), forceStringEx(r.value('name')), forceDouble(r.value('adultUetDoctor')))
            else: self._serviceInfoCache[code] = (None, None, None)
    ...
    def process(self, fileName):
        ...
        try:
            self._logDBExists = bool(self._db.table(self._logDBName+'.MainFile'))
    ...
    def parseFileName(self, fileName):
        ...
        try:
            ...
            if name[0] in ['h', 'l']:
                ...
                if len(orgs) < 14: raise
                if len(account) < 5: raise

                docType = orgs[0]
                sourceType = orgs[1]
                sourceCodeLength = 6 if sourceType == self.IS_MO else 5
                sourceCode = orgs[2:2+sourceCodeLength]
                destType = orgs[2+sourceCodeLength]
                destCodeLength = 6 if destType == self.IS_MO else 5
                destCode = orgs[3+sourceCodeLength:3+sourceCodeLength+destCodeLength]
    ...
    def processFile(self, fullFileName):
        ...
        try:
            ...
            persFileName = 'L'+fileName[1:] + '.zip'
            fullPersFileName = os.path.join(dirName, persFileName)
            logFileTable = self.logDBTable('MainFile') if self._logDBExists else None
            ...
            if logFileRecord:
                ...
                logFileId = self._db.insertRecord(self.logDBTable('MainFile'), logFileRecord) if logFileRecord else None
    ...
    def processDocument(self, document, sourceInfis, year, month, logFileRecord=None):
        ...
        try:
            ...
            logFileId = self._db.updateRecord(self.logDBTable('MainFile'), logFileRecord) if logFileRecord else None
    ...
    def processPersElement(self, persElement, logFileId = None):
        ...
        if logFileId:
            table = self.logDBTable('MainPers')
            record = table.newRecord()
    ...
    def processAccountElement(self, accountElement, sourceInfis, year, accInfo, logFileRecord):
        ...
        summav = self.getElementValue(accountElement, 'SUMMAV', typeName='double', isRequired=True, mekErrorList=mekErrorList)
        summap = self.getElementValue(accountElement, 'SUMMAP', typeName='double')
        ...
        if logFileRecord:
            ...
            logFileRecord.setValue('schet_summav', toVariant(summav))
    ...
    def processEntity(self, entityElement, accInfo, logFileId = None):
        ...
        try:
            MEKErrorList = []
            if logFileId:
                tableZap = self.logDBTable('MainZap')
    ...
    def processEvent(self, eventElement, patientInfo, accInfo, entityMekErrorList, zapId = None, logFileId = None):
        ...
        rsltCode = self.getElementValue(eventElement, 'RSLT', isRequired=True, mekErrorList=mekErrorList)
        specialityCode = self.getElementValue(eventElement, 'PRVS', isRequired=True, mekErrorList=mekErrorList)
        personCode = self.getElementValue(eventElement, 'IDDOKT', isRequired=True, mekErrorList=mekErrorList)
        idsp = self.getElementValue(eventElement, 'IDSP', typeName='int', isRequired=True, mekErrorList=mekErrorList)
        sluchId = None
        if zapId:
            sluchTable = self.logDBTable('MainSluch')
            ...
            sluchRecord.setValue('usl_ok', toVariant(self.convertValue(purposeCode, 'n')))
            sluchRecord.setValue('vidpom', toVariant(self.convertValue(aidKindCode, 'n')))
            sluchRecord.setValue('npr_mo', toVariant(self.getElementValue(eventElement, 'NPR_MO', return_null=True)))
            sluchRecord.setValue('extr', toVariant(self.getElementValue(eventElement, 'EXTR',
                                                                        typeName='n', return_null=True)))
            ...
            sluchRecord.setValue('podr', toVariant(self.getElementValue(eventElement, 'PODR',
                                                                        typeName='n', return_null=True)))
            sluchRecord.setValue('profil', toVariant(self.getElementValue(eventElement, 'PROFIL', typeName='n',
                                                                          isRequired=True, mekErrorList=mekErrorList)))
            sluchRecord.setValue('det', toVariant(self.getElementValue(eventElement, 'DET', typeName='n',
                                                                       isRequired=True, mekErrorList=mekErrorList)))
            ...
            sluchRecord.setValue('ds0', toVariant(self.getElementValue(eventElement, 'DS0', return_null=True)))
            sluchRecord.setValue('ds1', toVariant(self.getElementValue(eventElement, 'DS1',
                                                                       isRequired=True, mekErrorList=mekErrorList)))
            sluchRecord.setValue('ds2', toVariant(self.getElementValue(eventElement, 'DS2', return_null=True)))
            sluchRecord.setValue('ds3', toVariant(self.getElementValue(eventElement, 'DS3', return_null=True)))
            sluchRecord.setValue('vnov_m', toVariant(self.getElementValue(eventElement, 'VNOV_M',
                                                                          typeName='n', return_null=True)))
            sluchRecord.setValue('code_mes1', toVariant(self.getElementValue(eventElement, 'CODE_MES1',
                                                                             return_null=True)))
            sluchRecord.setValue('code_mes2', toVariant(self.getElementValue(eventElement, 'CODE_MES2',
                                                                             return_null=True)))
            sluchRecord.setValue('rslt', toVariant(rsltCode))
            sluchRecord.setValue('ishod', toVariant(diagnosticResultCode))
            sluchRecord.setValue('prvs', toVariant(specialityCode))
            sluchRecord.setValue('vers_spec', toVariant(self.getElementValue(eventElement, 'VERS_SPEC',
                                                                             return_null=True)))
            sluchRecord.setValue('iddokt', toVariant(personCode))
            #TODO: craz: теоретически, может быть несколько OS_SLUCH, и надо хранить каждый
            sluchRecord.setValue('os_sluch', toVariant(self.getElementValue(eventElement, 'OS_SLUCH', return_null=True)))
            ...
            if not sankElement.isNull():
                sankTable = self.logDBTable('MainSank')
        ...
        while not serviceElement.isNull():
            serviceList.append(self.prepareService(serviceElement, accInfo, mekErrorList, sluchId=sluchId, logFileId=logFileId))
        ...
        try:
            ...
            eventRecord = None
            is_new = True
            if not filter(lambda x: x['code'].startswith('4'), serviceList):
            ...
            if not filter(lambda x: x['uet'] != 0, serviceList) \
                and reduce(lambda x, y: x + y['amount'] if re.match('^3\..\.1\.', y['code']) and y['code'] not in ('3.1.1.171', '3.2.1.171', '3.1.1.089', '3.1.1.090') else x, serviceList, 0.0) == 1 \
                or reduce(lambda x, y: x + y['amount'] if re.match('^3\..\.2\.', y['code']) and y['code'] not in ('3.1.2.171', '3.2.2.171', '3.1.2.089', '3.1.2.090') else x, serviceList, 0.0) > 1:
                map(lambda x: x['mekErrorList'].append((u'6.18.7',
                                                        u'Несоответствие кода услуги диагнозу, полу, возрасту, профилю отделения',
                                                        forceStringEx(serviceElement.nodeName()),
                                                        'CODE_USL')), serviceList)
            ...
            for service in serviceList:
                ...
                # FIXME: проверять дату рождения из NOVOR
                ...
                if service['tariff_id']:
                    ...
                    if tariffType in (2, 5):
                        actionTypeId = self.getCachedRBValue('actionType', service['code'])
                        if not actionTypeId: return False
                        if serviceCode.startswith('3.'):
                            sum = price
                        else:
                            sum = round(price * amount, 2)
                        # Якобы устаревшие услуги без тарифа, должны быть отказаны.
                        if not price and serviceCode.startswith('3.'):
                            finalMekErrorList.append(((u'5.1.4',
                                                    u'Некорректное заполнение полей реестра счетов',
                                                    forceStringEx(serviceElement.nodeName()),
                                                    'SUMV_USL')))
                        ...
                        if not old_action:
                            ...
                            actionRecord.setValue('status', toVariant(ActionStatus.Done))
                        ...
                        if filter(lambda x: x['uet'] != 0, serviceList):
                            if u'стома' in service['serviceName']:
                        ...
                        elif serviceCode.startswith('3.') and u'стома' in service['serviceName']:
                            finalMekErrorList.append(((u'5.1.4',
                                                    u'Некорректное заполнение полей реестра счетов',
                                                    forceStringEx(serviceElement.nodeName()),
                                                    'CODE_USL')))
                        ...
                        # Если есть A16.12.004.009, то не считаем койкодни, а всегда оплачиваем по полному тарифу.
                        if avgDuration:
                            if serviceCode in ('2.1.1361', '2.2.1361', '2.1.1364', '2.2.1364') and \
                                ...
                                if eventLength <= round(avgDuration * 0.3, 0):
                                    ...
                                    and not filter(lambda x: x['code'] == u'A16.12.004.009', serviceList):
    ...
    def prepareService(self, serviceElement, accInfo, eventMekErrorList, sluchId=None, logFileId=None):
        ...
        service['code'] = self.getElementValue(serviceElement, 'CODE_USL', isRequired=True, mekErrorList=mekErrorList)
        service['amount'] = self.getElementValue(serviceElement, 'KOL_USL', typeName='double', isRequired=True, mekErrorList=mekErrorList)
        ...
        if sluchId:
            uslTable = self.logDBTable('MainUsl')
            ...
            uslRecord.setValue('podr', toVariant(self.getElementValue(serviceElement, 'PODR', typeName='n', return_null=True)))
            uslRecord.setValue('profil', toVariant(service['profileCode']))
            uslRecord.setValue('vid_vme', toVariant(self.getElementValue(serviceElement, 'VID_VME', return_null=True)))
            uslRecord.setValue('det', toVariant(service['child']))
            uslRecord.setValue('date_in', toVariant(service['begDate']))
            uslRecord.setValue('date_out', toVariant(service['endDate']))
            uslRecord.setValue('ds', toVariant(service['MKB']))
            uslRecord.setValue('code_usl', toVariant(service['code']))
            uslRecord.setValue('kol_usl', toVariant(service['amount']))
        ...
        serviceId, serviceName, uet = self.getServiceInfo(service['code'])
    ...
    def processPatient(self, patientElement, mekErrorList, zapRecord):
        ...
        if zapRecord:
            ...
            zapRecord.setValue('pacient_novor', toVariant(self.getElementValue(patientElement, 'NOVOR')))
            zapRecord.setValue('pacient_vnov_d',
                               toVariant(self.getElementValue(patientElement, 'VNOV_D', typeName='n', return_null=True)))
...
class CImportDialog(QtGui.QDialog):
    ...
    MainEngine = CImportR85TFEngine
    DDEngine = CImportR85DDTFEngine

    engineByFormat = {'HM': MainEngine,
                      'DP': DDEngine,
                      'DV': DDEngine,
                      'DO': DDEngine,
                      'DS': DDEngine,
    ...
    def retranslateUi(self):
        ...
        self.setWindowTitle(forceTr(u'Импорт. Крым. ТФОМС.', context))
    ...
    def onBrowseFileClicked(self):
        fileName = forceStringEx(QtGui.QFileDialog.getOpenFileName(self,
                                                                   ...
                                                                   u'Файлы (H*.zip DP*.zip DV*.zip DO*.zip '
