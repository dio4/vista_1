...
def exportR85SMO(widget, accountId, accountItemIdList):
...
class CExportWizard(QtGui.QWizard):
    ...
    def getTmpDir(self):
        if not self.tmpDir:
            self.tmpDir = QtGui.qApp.getTmpDir('R85SMO')
        return self.tmpDir


    def getFullXmlFileName(self, infisCode = '85000'):
        ...
        xmlLocalFileName = self.nameByInfis.get(infisCode, None)
        if xmlLocalFileName is None:
            (date, number, exposeDate, contractId,  payerId,  aDate) = getAccountInfo(self.accountId)
            lpuInfis = forceString(QtGui.qApp.db.translate('Contract', 'id', contractId, 'number'))

            xmlLocalFileName = os.path.join(self.getTmpDir(), \
                u'HM%sS%s_%s%d.xml' % (lpuInfis, infisCode, date.toString('yyMM'), self.page1.edtRegistryNumber.value()))
            self.nameByInfis[infisCode] = xmlLocalFileName

        return xmlLocalFileName


    def getFullXmlPersFileName(self, infisCode):
        return os.path.join(self.getTmpDir(), u'L%s' % os.path.basename(self.getFullXmlFileName(infisCode))[1:])

    def getAllNames(self):
        names = self.nameByInfis.values()
        names.extend([self.getFullXmlPersFileName(infisCode) for infisCode in self.nameByInfis])
        return names
...
class CExportPage1(QtGui.QWizardPage, Ui_ExportPage1, CExportHelperMixin):
    ...
    def __init__(self, parent):
        ...
        self.ignoreErrors = forceBool(getVal(QtGui.qApp.preferences.appPrefs,
            'ExportR85SMOIgnoreErrors', False))
    ...
    def prepareToExport(self):
        ...
        querySums = self.createSumsQuery()
        self.progressBar.setMaximum(max(query.size(), 1))
        self.progressBar.reset()
        self.progressBar.setValue(0)
        return query, querySums
    ...
    def export(self):
        ...
        if self.aborted or not result:
            ...
            QtGui.qApp.preferences.appPrefs['ExportR85SMOIgnoreErrors'] = \
                    toVariant(self.chkIgnoreErrors.isChecked())
            QtGui.qApp.preferences.appPrefs['ExportR85SMOVerboseLog'] = \
                    toVariant(self.chkVerboseLog.isChecked())
            QtGui.qApp.preferences.appPrefs['ExportR85SMOGroupByService'] = \
                    toVariant(self.chkGroupByService.isChecked())
            QtGui.qApp.preferences.appPrefs['ExportR85SMOServiceInfoFileName'] = \
...
# *****************************************************************************************
    ...
    def createQuery(self):
        ...
        if self.chkGroupByService.isChecked():
            sumStr = """SUM(CAST(Account_Item.`sum` as DECIMAL(20, 10))) AS `sum`,
        ...
        stmt = u"""SELECT
                ...
                IF(Account_Item.refuseType_id IS NOT NULL, 2, 1) AS OPLATA, -- Полный отказ при наличии причины отказа
                ...
                Diagnostic.speciality_id AS diagnosticSpecialityId,
                ...
                MesMedicalAidProfile.federalCode as mesMedicalAidProfileFederalCode,
                FirstVisitMedicalAidProfile.federalCode as firstVisitProfileFedCode,
                IF(Account_Item.service_id IS NOT NULL,
                    rbItemService.id,
                ...
                rbPayRefuseType.flatCode as refuseCode
            FROM Account_Item
            ...
            LEFT JOIN rbMedicalAidProfile AS FirstVisitMedicalAidProfile ON FirstVisitMedicalAidProfile.id = (
                SELECT map.id FROM Visit v
                INNER JOIN rbService s ON s.id = v.service_id
                INNER JOIN rbMedicalAidProfile map ON map.id = s.medicalAidProfile_id
                WHERE v.event_id = Event.id AND v.deleted = 0
            ...
            LEFT JOIN rbService rbMesService ON rbMesService.code = MES.code
            LEFT JOIN rbMedicalAidProfile AS MesMedicalAidProfile ON
                MesMedicalAidProfile.id = rbMesService.medicalAidProfile_id
            WHERE
            ...
            --     )
            AND %s ORDER BY Insurer.`infisCode`, `Client`.lastName, `Client`.firstName, `Client`.patrName, Event.execDate""" % (sumStr, cond)

        query = db.query(stmt)
        return query

    def createSumsQuery(self):
        db = QtGui.qApp.db
        tableAccountItem = db.table('Account_Item')
        cond = tableAccountItem['id'].inlist(self.idList)
        ...
        stmt = """SELECT
                ...
                SUM(IF(Account_Item.refuseType_id IS NULL, Account_Item.sum, 0)) AS payedSum,
                SUM(IF(Account_Item.refuseType_id IS NOT NULL, Account_Item.sum, 0)) AS refusedSum,
                Insurer.infisCode AS insurerInfis
            FROM Account_Item
            LEFT JOIN Event  ON Event.id  = Account_Item.event_id
            ...
            #     OR (Account_Item.date IS NOT NULL AND rbPayRefuseType.rerun != 0)
            #    )
            AND %s GROUP BY insurerInfis""" % cond
...
# *****************************************************************************************
    ...
    def exportInt(self):
        ...
        # Подготовка
        self.recNum = 1
        query, querySums = self.prepareToExport()
        ...
        sums = {}
        while querySums.next():
            sumsRecord = querySums.record()
            sums[forceString(sumsRecord.value('insurerInfis'))] = (forceDouble(sumsRecord.value('sum')),
                                                                forceDouble(sumsRecord.value('payedSum')),
                                                                forceDouble(sumsRecord.value('refusedSum'))
                                                                )

        # Получаем идентификаторы плательщиков-получателей
        ...
        if self.idList:
            ...
            lpuCode = ''
            while query.next():
                infis = forceString(query.record().value('insurerInfis'))
                if infis != currentInfis and infis in ('85001', '85002', '85003'):
                    if not currentInfis is None:
                        out.writeFileFooter()
                        clientsOut.writeFileFooter()
                    currentInfis = infis
                    outFile, clientsFile = self.createXML(infis)
                    ...
                    lpuCode = forceString(QtGui.qApp.db.translate(
                        'Contract', 'id', contractId, 'number'))
                    out.writeFileHeader(outFile, self.parent.getFullXmlFileName(infis), accNumber, aDate,
                                        forceString(accDate.year()),
                                        forceString(accDate.month()),
                                        lpuCode, currentInfis, sums.get(infis, (0.0, 0.0, 0.0)))
                    clientsOut.writeFileHeader(clientsFile, self.parent.getFullXmlFileName(infis), aDate)
                    outFiles.extend([outFile, clientsFile])
                QtGui.qApp.processEvents()
                if self.aborted:
                    break
                self.progressBar.step()
                out.writeRecord(query.record(), lpuCode)
...
# *****************************************************************************************
    ...
    def createXML(self, infisCode):
        outFile = QtCore.QFile(self.parent.getFullXmlFileName(infisCode))
        outFile.open(QtCore.QFile.WriteOnly | QtCore.QFile.Text)
        outFile2 = QtCore.QFile( self.parent.getFullXmlPersFileName(infisCode))
...
class CExportPage2(QtGui.QWizardPage, Ui_ExportPage2):
    def __init__(self, parent):
        ...
        exportDir = forceString(getVal(QtGui.qApp.preferences.appPrefs, 'R85SMOExportDir', homePath))
    ...
    def validatePage(self):
        ...
        for src in self.wizard().getAllNames(): #(self.parent.getFullXmlFileName(),  self.parent.getFullXmlPersFileName()):
        ...
        if success:
            QtGui.qApp.preferences.appPrefs['R85SMOExportDir'] = toVariant(self.edtDir.text())
            QtGui.qApp.preferences.appPrefs['ExportR85SMORegistryNumber'] = \
...
class CMyXmlStreamWriter(QXmlStreamWriter, CExportHelperMixin):
    ...
    sluchFields = {'IDCASE': True, 'USL_OK': True, 'VIDPOM': True, 'FOR_POM': True, 'NPR_MO': False, 'EXTR': False,
                   'LPU': True, 'LPU_1': False, 'PODR': False, 'PROFIL': True, 'DET': True, 'NHISTORY': True,
                   'DATE_1': True, 'DATE_2': True, 'DS0': False, 'DS1': True, 'DS2': False, 'DS3': False,
                   'VNOV_M': False, 'CODE_MES1': False, 'CODE_MES2': False, 'RSLT': True, 'ISHOD': True, 'PRVS': True,
                   'VERS_SPEC': False, 'IDDOKT': True, 'OS_SLUCH': False, 'IDSP': True, 'ED_COL': False, 'TARIF': False,
                   'SUMV': True, 'OPLATA': False, 'SUMP': False, 'SANK_IT': False, 'COMENTSL': False}
    uslFields = {'IDSERV': True, 'LPU': True, 'LPU_1': False, 'PODR': False, 'PROFIL': True, 'VID_VME': False,
                 'DATE_IN': True, 'DATE_OUT': True, 'DS': True, 'CODE_USL': True, 'KOL_USL': True,
                 'TARIF': False, 'SUMV_USL': True, 'PRVS': True, 'CODE_MD': True, 'COMENTU': False}
    sankFields = {'S_CODE': True, 'S_SUM': True, 'S_TIP': True, 'S_OSN': True, 'S_COM': False, 'S_IST': True}
    groupMap = {'ZGLV': zglvFields, 'SCHET': schetFields, 'PACIENT': pacientFields, 'SLUCH': sluchFields, 'USL': uslFields, 'SANK': sankFields}
    ...
    def __init__(self, parent):
        ...
        self.clientStr = u''
        self.extraSums = forceStringEx(self.parent.edt85001.text()) + forceStringEx(self.parent.edt85002.text()) + forceStringEx(self.parent.edt85003.text()) # Ужас. Вроде как временное решение, так что пофиг - если у нас хоть одно из полей заполнено, считаем, что подушевые суммы есть. Снести при первой возможности.
    ...
    def writeRecord(self, record, lpuCode):
        ...
        self.writeTextElement('LPU', '%s' % lpuCode[:6]) #460
        ...
        if codeUsl[0] == '6': #forceString(record.value('eventTypeCode')) == '5':
            profileStr = forceString(record.value('firstVisitProfileFedCode'))
        else:
            ...
            if not profileStr:
                profileStr = forceString(record.value('mesMedicalAidProfileFederalCode'))[:3]
        profileStr = profileStr.lstrip('0')
        ...
        prvs = forceString(self.parent.getSpecialityFederalCode(personId))
        if not prvs:
            prvs = forceString(QtGui.qApp.db.translate('rbSpeciality', 'id', record.value('diagnosticSpecialityId'), 'federalCode'))
        self.writeTextElement('PRVS', prvs)
        self.writeTextElement('IDDOKT', forceString(self.parent.getPersonFederalCode(personId)))
        idsp = forceString(record.value('medicalAidUnitFederalCode'))
        if not sum and self.extraSums:
            if codeUsl.startswith('3'):
                idsp = '26'
            elif codeUsl.startswith('4'):
                idsp = '35'
        self.writeTextElement('IDSP', idsp)
        if forceDouble(record.value('uet')):
            self.writeTextElement('ED_COL', forceString(record.value('uet')))
        else:
        ...
        refuseCode = forceString(record.value('refuseCode'))
        self.writeTextElement('SUMP', ('%.2f' % (sum if not refuseCode else 0)))

        if refuseCode:
            self.writeStartElement('SANK')
            self.writeTextElement('S_CODE', '1')
            self.writeTextElement('S_SUM', ('%.2f' % sum))
            self.writeTextElement('S_TIP', '1')
            self.writeTextElement('S_OSN', refuseCode)
            self.writeTextElement('S_IST', '1')
            self.writeEndElement()          # SANK
        ...
        if serviceId:
            self.writeService(record, lpuCode, age, price, sum, codeUsl, profileStr)
        #if federalPrice > 0:
    ...
    def writeService(self, record, lpuCode, age, price, sum, code, profileStr):
        ...
        self.writeTextElement('LPU', '%s' % lpuCode[:6]) #O Т(6) Код МО МО лечения #460
        ...
        prvs = forceString(self.parent.getSpecialityFederalCode(forceRef(record.value('execPersonId'))))
        if not prvs:
            prvs = forceString(QtGui.qApp.db.translate('rbSpeciality', 'id', record.value('diagnosticSpecialityId'), 'federalCode'))
        self.writeTextElement('PRVS', prvs) #O N(9) Специальность медработника, выполнившего услугу
    ...
    def writeFileHeader(self,  device, fileName, accNumber, accDate, year, month, infisCode, payerCode, accSum=(0.0, 0.0, 0.0)):
        self._lastClientId = None
        self.recNum = 1
        self.caseNum = 1
        self.serviceNum = 1
    ...
    def writeHeader(self, fileName, accNumber, accDate, year, month, infisCode, payerCode, accSum):
        ...
        total = accSum[0]
        payed = accSum[1]
        if payerCode[:5] == '85001' and forceString(self.parent.edt85001.text()):
            total += forceDouble(self.parent.edt85001.text())
            payed += forceDouble(self.parent.edt85001.text())
        elif payerCode[:5] == '85002' and forceString(self.parent.edt85001.text()):
            total += forceDouble(self.parent.edt85002.text())
            payed += forceDouble(self.parent.edt85002.text())
        elif payerCode[:5] == '85003' and forceString(self.parent.edt85001.text()):
            total += forceDouble(self.parent.edt85003.text())
            payed += forceDouble(self.parent.edt85003.text())
        self.writeTextElement('SUMMAV', ('%.2f' % total))
        self.writeTextElement('SUMMAP', ('%.2f' % payed))
        self.writeTextElement('SANK_MEK', ('%.2f' % accSum[2]))
        self.writeEndElement()
