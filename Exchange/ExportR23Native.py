...
from library.Utils import calcAgeInDays, calcAgeInYears, calcAgeTuple, forceBool, forceDate, forceDecimal, forceDouble, \
    ...
    forceStringEx, formatSNILS, formatSex, getEventProfileForService, getVal, monthName, nameCase, pyDate, toVariant
...
def getWeightIfAborted(eventId):
    db = QtGui.qApp.db
    query = db.getRecordList(table=None, stmt=u'''
    SELECT Event_LittleStranger.birthWeight
    FROM Event
        INNER JOIN Event_LittleStranger ON Event.littleStranger_id = Event_LittleStranger.id
    ...
    WHERE
        Diagnostic.deleted = 0 AND Diagnosis.deleted = 0 AND Event.id = %i AND rbDiagnosisType.code = 9
        AND Diagnosis.MKB IN ('P05.0', 'P05.1', 'P05.2', 'P05.9', 'P07.0', 'P07.1', 'P07.2', 'P07.3')''' % eventId)
    if len(query) > 0:
        return forceDouble(query[0].value(0))
    else:
        return None
...
def exportR23Native(widget, accountId, accountItemIdList):
...
class CBaseExportPage1(QtGui.QWizardPage, Ui_ExportR23NativePage1, CExportHelperMixin):
    ...
    def __init__(self, parent):
        ...
        self.chkVerboseLog.setChecked(
            forceBool(QtGui.qApp.preferences.appPrefs.get('ExportR23NativeVerboseLog', 'False')))
    ...
    def export(self):
        try:
            ...
            QtGui.QMessageBox.critical(self, u'Ошибка', u'Необходимо указать числовой код подразделения.',
    ...
    def getDbfBaseName(self, modern=False):
    ...
    def getPayerCode(self):
        ...
        payerCode = forceString(
            QtGui.qApp.db.translate('Organisation', 'id', payerId, 'infisCode')) if payerId else u'000'
        ...
        if (len(accSplit) > 1):
            payerCode2 = forceString(
                QtGui.qApp.db.translate(u'Organisation', u'infisCode', forceInt(accSplit[-1]), u'infisCode'))
            if payerCode2 != '':
    ...
    def getAccountUET(self, accItemId):
        db = QtGui.qApp.db
        tableAccount = db.table('Account')
        tableAccountItem = db.table('Account_Item')
        record = db.getRecordEx(
            tableAccount.innerJoin(tableAccountItem, [tableAccountItem['master_id'].eq(tableAccount['id'])]),
            tableAccount['uet'],
            [tableAccountItem['id'].eq(accItemId),
    ...
    def getInvoiceData(self, accNumber):
        ...
        result = {}
        if dbf_u:
            result['accountUET'] = self.getAccountUET(dbf_u[0]['UID'])
        ...
        for rec_u in dbf_u:
            ...
            currVpData = result.setdefault(currRow,
                                           {'tsumm': 0.0, 'eventSet': set(), 'uet': 0.0, 'visits': 0, 'tkd': 0})
            currVpData['tsumm'] += rec_u['SUMM']
    ...
    def makeInvoice(self, accNumber, dats, isOutArea, accType, datps, codeLpu):
        ...
        reps, pogrn = self.getInvoiceData(accNumber)
        ...
        payerId = None
        if r: payerId = forceRef(r.value('id')) if not forceRef(record.value('head_id')) else forceRef(
        ...
        if len(pogrn) == 1 and not isOutArea:
            ...
            record = db.getRecordEx(tableOrganisation,
                                    ...
                                    [tableOrganisation['OGRN'].eq(ogrn),
                                     ...
                                     # Код Краснодарского Края #TODO:skkachaev: сделать по-человечески
        ...
        invoice_cursor.insertHtml(u"к реестру счетов № %s от %s за %s %s %s<br>" % (
        invoice_num, invoice_date, str(datps.year()), zamonth, accName))
        tableColumns = [
        ...
        invoice_cursor.movePosition(QtGui.QTextCursor.End)
        invoice_cursor.insertHtml(
            u"<br><br>" + u"Главный врач __________ %30s." % forceString(record.value('chief')) + u"&nbsp;" * 25 +
...
class CExportPage1(CBaseExportPage1):
    ...
    def exportInt(self):
        ...
        accOrgStructureId = forceRef(QtGui.qApp.db.translate('Account', 'id', self.parent.accountId,
                                                             'orgStructure_id')) or QtGui.qApp.currentOrgStructureId()
        ...
        strContractNumber = forceString(
            QtGui.qApp.db.translate('Contract', 'id', contractId, 'number')) if contractId else u'б/н'
    ...
    def getEventInfo(self):
        ...
        WHERE Account_Item.reexposeItem_id IS NULL
            ...
            AND %s
        GROUP BY Event.id;""" % tableAccountItem['id'].inlist(self.idList)
        query = db.query(stmt)
        result = {}
    ...
    def process(self, dbf, record, bookkeeperCode, accNumber, settleDate, accType, payerOGRN):
        ...
        outgoingRefNumber = forceString(record.value('referralNumber'))  # 'outgoingRefNumber'))
        policRefOrgId = forceRef(record.value('policRefOrgId'))
        if policRefOrgId:
            outgoingOrgCode = forceInt(db.translate('Organisation', 'id', policRefOrgId, 'infisCode'))
        ...
        # filling dictionary with all possible KSOs from file U
        ...
        if not ((eventId, modern, externalId) in self.exportedClients):
            ...
            DATO = forceDate(record.value('servEndDate'))
            ...
            dats = self.edtDats.date()
            dbfRecord['DATS'] = pyDate(
                dats if dats and dats.isValid() else settleDate if settleDate and settleDate.isValid() else QtCore.QDate.currentDate())
            ...
            datps = self.edtDatps.date()
            dbfRecord['DATPS'] = pyDate(
                datps if datps and datps.isValid() else settleDate if settleDate and settleDate.isValid() else QtCore.QDate.currentDate())
            ...
            insuranceArea = forceString(
                db.translate('kladr.KLADR', 'CODE', forceString(record.value('policyInsuranceArea')), 'OCATD',
                             idFieldName='CODE'))
            if not insuranceArea:
                self.log(
                    u'<b><font color=orange>Внимание</font></b>: в полисе не задана территория страхования, пытаюсь определить по ОКАТО СМО!')

                insuranceArea = forceString(record.value('insurerOKATO'))
                if not insuranceArea:
                    insurerName = forceString(
                        db.translate('kladr.KLADR', 'CODE', insurerArea, 'OCATD', idFieldName='CODE'))
                    self.log(
                        u'<b><font color=orange>Внимание</font></b>: ОКАТО для СМО "%s" не задан, пытаюсь определить по области страхования!' % insurerName)

                    insuranceArea = forceString(
                        db.translate('kladr.KLADR', 'CODE', insurerArea, 'OCATD', idFieldName='CODE'))
                    if not insuranceArea:
                        self.log(
                            u'<b><font color=orange>Внимание</font></b>: Область страхования для СМО "%s" не задана!' % insurerName)
            ...
            # if representativeInfo and (exportAsLittleStranger or isLittleStrangerAge):
            ...
            if forceString(referralNumber) and not forceInt(record.value('referralIsOutgoing')):
                ...
                if not forceInt(db.translate(tblVisitType, tblVisitType['id'], forceInt(record.value('visitTypeId')),
                                             tblVisitType['code'])) == 15:
                    # номер направления (п.15 примечаний) обязательное (по направлениям на плановую госпититализацию)
                    dbfRecord['NAPR_N'] = '%.5d_%s' % (setOrgCode,
                                                       referralNumber) if '_' not in referralNumber else referralNumber  # '%.5d_%.7d' % (forceInt(self.edtOrgStructureCode.text()), referralNumber)
            ...
            dbfRecord['MP'] = '8' if forceInt(record.value('order')) in [5, 8] else '2' if forceInt(
                record.value('order')) == 2 else '1'
            ...
            if vmpFromPerson:
                ...
                tblMedicalAidProfile = db.table('rbMedicalAidProfile')
                ...
                tblQuery = tblQuery.leftJoin(tblMedicalAidProfile, tblMedicalAidProfile['id'].eq(tblService['medicalAidProfile_id']))
                ...
                recCode = db.getRecordEx(tblQuery, [tblMedicalAidKind['federalCode'], tblMedicalAidUnit['code'], tblMedicalAidProfile['federalCode'].alias('ProfilFederalCode')], cond)
                ...
                if recCode:
                    ...
                    if forceString(recCode.value('ProfilFederalCode')):
                        dbfRecord['PROFIL'] = forceString(recCode.value('ProfilFederalCode'))
            ...
            if dbfRecord['MP'] == '2' and (dbfRecord['KSO'] == "29"):
                dbfRecord['P_CEL'] = '1'
            if dbfRecord['MP'] == '8':
                dbfRecord['P_CEL'] = '2'
            ...
            u""" i3393 P file
                3. Добавить после поля INV поле P_PER, С (1) - признак поступления/перевода

                Обязательно для заполнения для стационаров всех типов.

                Допустимые значения:
                1-Самостоятельно: если в действии "Поступление" -> "Кем доставлен" указано "Самостоятельно";
                2-СМП: если в действии "Поступление" -> "Кем доставлен" указано "СМП";
                3-Перевод из другой МО: если указан "Направитель" на вкладке "Примечания";
                4-Перевод внутри МО с другого профиля: если в действии "Движение" -> "Переведен из отделения" указано
                отделение данной МО.
                Для остальных случаев поле не заполняется.
            """
            # if u"Самостоятельно" in forceString(record.value('whoDelivered')):
            #     dbfRecord['P_PER'] = 1  # u"Самостоятельно"
            # elif u"СМП" in forceString(record.value('whoDelivered')):
            #     dbfRecord['P_PER'] = 2  # u"СМП"
            # if forceRef(record.value('whoRelegate')):  # relegateOrg
            #     dbfRecord['P_PER'] = 3  # u"Перевод из другой МО"
            # if forceRef(record.value('movingFromOrg')):
            #     dbfRecord['P_PER'] = 4  # u"Перевод внутри МО с другого профиля"
            ...
            Для счетов по диспансеризации; типы: a,c,d,e,g,h. -> dbfRecord['VS']
            ...
            if forceInt(record.value('eventProfileCode')) in [211, 232, 252, 261, 262]:  # and accType in 'acdegh':
        ...
        # ======================================================================================
        # U FILE
        # i3882: to export to U file unique rows
        if accountItemId not in self.exportedUIDs:
            self.exportedUIDs.append(accountItemId)
            ...
            dbfRecord['SN'] = eventId  # clientId #eventId
            ...
            if forceString(record.value('servOrgStructCode')):
            ...
            u""" i3716 U file
            Поле MKBХ_PR: только для счетов по диспансеризации/профилактическим медицинским осмотрам (типы счетов: a,c,d,e,g,h)
            Форматы экспорта счетов R23NATIVE, R23DKKB.

            Если для диагноза MKBX (Тип - "Заключительный")
            в столбце "Хар" указан код "2" (хроническое впервые установленное)
            то выгружаем MKBХ_PR=1 (диагноз выявлен впервые).

            4.2. Поле MKBХS_PR: только для счетов по диспансеризации/профилактическим медицинским осмотрам (типы счетов: a,c,d,e,g,h)
            Форматы экспорта счетов R23NATIVE, R23DKKB.

            Если для диагноза MKBXS (Тип - "Сопутствующий")
            в столбце "Хар" указан код "2" (хроническое впервые установленное)
            то выгружаем MKBХS_PR=1 (диагноз выявлен впервые).

            Если MKBХ или MKBХS равны Z.00 (заболеваний не выявлено) или Z.01.4 (гинекология, без жалоб),
            то для этих диагнозов MKBХ_PR и MKBХS_PR не заполняем.
            """

            if accType in 'acdegh' and forceString(record.value('eventTypeCode')) in ['06', '211', '232', '252', '261',
                                                                                      '262']:
                primaryQuery = u"""
                SELECT
            ...
            dbfRecord['VP'] = getEventProfileForService(
                forceRef(record.value('serviceId')),
                begDate.toString('yyyy-MM-dd'),
                forceRef(record.value('specialityId')),
                birthDate.toString('yyyy-MM-dd'),
                forceInt(record.value('sex')),
                forceString(record.value('MKB'))
            ) or forceString(record.value('eventProfileCode'))
            ...
            # [OLD] признак: услуга оказана в другой МО (п. 3 примечаний) обязательное
            # [NEW] Причины невыполнения медицинской услуги (i3393)
            u""" i3393 U file
            ...
            if dbfRecord[
                'IS_OUT'] == 0 and status == ActionStatus.DoneWhilstClinicalExamination and not isExternalService:
...
class CExportPage2(QtGui.QWizardPage, Ui_ExportR23NativePage2):
    ...
    def validatePage(self):
        ...
        filePath = os.path.join(tmpDir, os.path.basename(htmlFileName))
        open(filePath, 'a').close()  # create file if doesn't exist
        zf.write(filePath, htmlFileName, ZIP_DEFLATED)
        if os.path.exists(os.path.join(tmpDir, os.path.basename(html2FileName))):
            filePath = os.path.join(tmpDir, os.path.basename(html2FileName))
            open(filePath, 'a').close()  # create file if doesn't exist
            zf.write(filePath, html2FileName, ZIP_DEFLATED)
        zf.close()
