...
# Форма 027: форма ввода "Протокол"
...
from Events.EventInfo import CCureMethodInfo, CCureTypeInfo, CDiagnosticInfoProxyList, CEventInfo, CEventTypeInfo, \
    CPatientModelInfo, CResultInfo
from Events.Utils import CTableSummaryActionsMenuMixin, checkDiagnosis, checkIsHandleDiagnosisIsChecked, \
    getAvailableCharacterIdByMKB, getDiagnosisId2, getDiagnosisSetDateVisible, getEventContext, \
    getEventContextData, getEventDuration, getEventLengthDays, getEventMesRequired, getEventName, getEventPeriodEx, \
    getEventPurposeId, getEventServiceId, getEventShowTime, \
...
from Orgs.Utils import COrgInfo, getOrganisationShortName, getPersonChiefs
from Registry.HospitalizationTransferDialog import CHospitalizationTransferDialog
from Registry.Utils import CClientInfo, CQuotaTypeInfo
from Ui_F027 import Ui_Dialog
...
from library.PrintInfo import CDateInfo, CDateTimeInfo, CInfoContext, CInfoProxyList
...
class CProtocolEventEditDialog(CEventEditDialog):
    def getEventInfo(self, context, infoClass=CEventInfo):
        ...
        result._order = self.cmbOrder.currentIndex() if hasattr(self, 'cmbOrder') else 0
        ...
        result._quotaType = context.getInstance(CQuotaTypeInfo, self.getQuotaTypeId())
    ...
    def getExternalId(self):
        return forceString(self.cmbEventExternalIdValue.currentText())
...
class CF027Dialog(CProtocolEventEditDialog, Ui_Dialog, CTableSummaryActionsMenuMixin):
    ...
    def __init__(self, parent):
        ...
        self.modelAPPlanningActionProperties = CActionPropertiesTableModel(self)
        ...
        # self.cmbQuota.setClientId(self.clientId)
        # self.cmbQuota.setBegDate(QtCore.QDate().currentDate())
        # self.cmbQuota.setEndDate(QtCore.QDate().currentDate())
    ...
    def getEditable(self):
        ...
        if self._isCheckPresenceInAccounts:
            editable = editable and not isEventPresenceInAccounts(forceRef(self._record.value('id'))) or QtGui.qApp.userHasRight(urEditPayedEvents)
        return editable
    ...
    def setEditable(self, editable):
        ...
        # self.tabMisc.tblAPActions.setEnabled(editable)
        # self.tabMisc.tblAPProps.setEnabled(editable)
        # self.tabMisc.frameAP.setEnabled(editable)

        # self.tabInspectionPlan.setEditable(editable)
        # self.tabInspectionPlan.tblAPActions.setEnabled(editable)
        # self.tabInspectionPlan.tblAPProps.setEnabled(editable)
        # self.tabInspectionPlan.frameAP.setEnabled(editable)

        self.tabAmbCard.setEnabled(editable)
        self.tabNotes.setEnabled(editable)
        #self.tabReferral.setEnabled(editable)
    ...
    def createAction(self):
        ...
        if self.clientId:
            ...
            if action:
                ...
                if status == ActionStatus.Done and (personId or personManagerId or personMedicineHeadId or eventCuratorId or eventAssistantId):  # or personExpertId:
    ...
    def _prepare(self, contractId, clientId, eventTypeId, orgId, personId, eventSetDatetime, eventDatetime, includeRedDays, numDays,
        ...
        head = db.getRecordEx(stmt='SELECT p.id AS id FROM Person p INNER JOIN rbPost rbp ON p.post_id = rbp.id WHERE rbp.code = \'2005\' LIMIT 0, 1')
        if head:
            self.cmbPersonMedicineHead.setValue(forceRef(head.value('id')))
        assistant = db.getRecordEx(stmt='SELECT p.id AS id FROM Person p INNER JOIN rbPost rbp ON p.post_id = rbp.id WHERE rbp.code = \'2006\' LIMIT 0, 1')
        if assistant:
    ...
    def prepare(self, clientId, eventTypeId, orgId, personId, eventSetDatetime, eventDatetime, includeRedDays, numDays,
        ...
        if QtGui.qApp.userHasRight(urAccessF025planner):
            ...
            return self._prepare(None, clientId, eventTypeId, orgId, personId, eventSetDatetime, eventDatetime, includeRedDays, numDays,
                                 ...
                                 externalId, assistantId, curatorId, None, [], relegateOrgId, diagnos, financeId, protocolQuoteId, actionByNewEvent, isAmb=isAmb, recommendationList=recommendationList)
        ...
        if eventId:
            ...
            if record:
                ...
                if patientModelId:
                    ...
                    if not self.cmbPatientModel.isObsolete:
                ...
                if self.cmbPatientModel.isObsolete:
                    self.cmbCureMethod.setEnabled(False)
                    self.cmbCureType.setEnabled(False)
    ...
    def setRecord(self, record):
        ...
        # если действие планирования существует, загружаем его
        tableAction = db.table('Action')
        actionTypeIdList = getActionTypeIdListByFlatCode(u'planning')
        actionTypeId = actionTypeIdList[0] if len(actionTypeIdList) else None
        cond = [
            tableAction['deleted'].eq(0),
            tableAction['event_id'].eq(forceRef(record.value('id'))),
            tableAction['actionType_id'].eq(actionTypeId)
        ]
        record = db.getRecordEx(tableAction, '*', cond)
        if record:
            action = CAction(record=record)
            self.modelAPPlanningActionProperties.setAction(action, self.clientId, self.clientSex, self.clientAge)
            self.edtPlanningDate.setDate(forceDate(record.value('directionDate')))
            self.oldPlanningDate = forceDate(record.value('directionDate'))
            self.edtPlanningDate.setEnabled(True)
...
#перенести в exec_ в случае успеха или в accept?
    ...
    def setPlanningOrgStructure(self):
        db = QtGui.qApp.db
        ...
        if orgStructureId:
            self.modelAPPlanningActionProperties.action[u'подразделение'] = orgStructureId

    @QtCore.pyqtSlot(int)
    def onCmbResultCurrentIndexChanged(self, index):
        if self.notSetCmbResult and self.cmbResult.code() == '1':
            if not self.modelAPPlanningActionProperties.action:
                db = QtGui.qApp.db
                tableActionType = db.table('ActionType')
                idListActionType = db.getIdList(tableActionType, [tableActionType['id']],
                                                [tableActionType['flatCode'].like(u'planning%'),
                                                 tableActionType['deleted'].eq(0)])
                if not idListActionType:
                    return
                action = CAction.createByTypeId(idListActionType[0])
    ...
    def on_cmbPatientModel_currentIndexChanged(self, index):
        ...
        if cureTypeIdList:
            self.cmbCureType.setFilter('rbCureType.id IN (%s) AND rbCureType.isObsolete = 0'%(u','.join(str(cureTypeId) for cureTypeId in cureTypeIdList if cureTypeId)))
            ...
            if cureMethodIdList:
                self.cmbCureMethod.setFilter('rbCureMethod.id IN (%s) AND rbCureMethod.isObsolete = 0'%(u','.join(str(cureMethodId) for cureMethodId in cureMethodIdList if cureMethodId)))
            else:
                cond = [tablePatientModelItem['master_id'].eq(patientModelId)]
                cureMethodIdList = db.getDistinctIdList(tablePatientModelItem, [tablePatientModelItem['cureMethod_id']], cond)
                if cureMethodIdList:
    ...
    def saveInternals(self, eventId):
        super(CF027Dialog, self).saveInternals(eventId)
...
#        specialityIdVariant = QtGui.qApp.db.translate('Person', 'id', personIdVariant, 'speciality_id')
    ...
    def getPlanningAction(self, eventId = None, save=False):
        planningAction = self.modelAPPlanningActionProperties.action
        if planningAction:
            personId = self.cmbPersonManager.value()
            orgStructureId = forceRef(
                QtGui.qApp.db.translate('Person', 'id', personId, 'orgStructure_id')) if personId else None
            if orgStructureId:
                planningAction[u'подразделение'] = orgStructureId

            if save:
                planningAction.save()

            record = planningAction._record
            record.setValue('event_id', toVariant(self.itemId() if self.itemId() else eventId))
            record.setValue('begDate', toVariant(QtCore.QDateTime(self.edtBegDate.date(), self.edtBegTime.time())))
            record.setValue('endDate', toVariant(QtCore.QDateTime(self.edtEndDate.date(), self.edtEndTime.time())))
            record.setValue('directionDate', toVariant(self.edtPlanningDate.date()))
            record.setValue('person_id', toVariant(self.cmbPersonManager.value()))
            record.setValue('setPerson_id', toVariant(self.cmbPersonMedicineHead.value()))
            # action.setAssistant('assistant', self.cmbPersonExpert.value())
            if save:
    ...
    def saveAPropertys(self, eventId):
        ...
        self.getPlanningAction(eventId, save=True)
    ...
    def checkDataEntered(self):
        if self.itemId() and self.oldPlanningDate and self.oldPlanningDate != self.edtPlanningDate.date():
            dialog = CHospitalizationTransferDialog(parent=self, eventId=self.itemId())
            dialog.edtTransferDate.setDate(self.edtPlanningDate.date())
            if dialog.exec_():
                self.oldPlanningDate = self.edtPlanningDate.date()
            else:
                QtGui.QMessageBox.warning(
                    self,
                    u'Внимание!',
                    u'Сохранение обращения не возможно без указания причины переноса госпитализации с %s на %s.' % (
                        self.oldPlanningDate.toString('dd.MM.yyyy'),
                        self.edtPlanningDate.date().toString('dd.MM.yyyy')
                    ),
        ...
        if not QtGui.qApp.userHasRight(urDoNotCheckResultAndMKB):
            result = result and (endDate.isNull() or self.cmbResult.value() or self.checkInputMessage(u'результат', True, self.cmbResult))
        if endDate.isNull():
            pass
        else:
            ...
            if minDuration<=maxDuration:
                ...
                result = result and (maxDuration==0 or eventDuration <= maxDuration or
                                     self.checkValueMessage(u'Длительность должна быть не более %s. %s'%(formatNum(maxDuration, (u'дня', u'дней', u'дней')), eventDurationErrorString), False, self.edtEndDate))        #result = result and (len(self.modelFinalDiagnostics.items())>0 or self.checkInputMessage(u'диагноз', False, self.tblFinalDiagnostics))
        ...
        # result = result and self.checkActionsPlanningDataEntered([self.tabMisc])  # , self.tabInspectionPlan])
        ...
        if \
                ...
                and QtGui.qApp.isTNMSVisible() and (self.modelFinalDiagnostics.getFinalDiagnosis().value('TNMS') is None or
    ...
    def checkActionsPlanningDataEntered(self, tabList):
        return self.edtPlanningDate.date().isValid()
    ...
    def getEventInfo(self, context, infoClass=CEventInfo):
        ...
        planningAction = self.getPlanningAction()

        result._actions = []
        if protocolAction:
            result._actions.append(context.getInstance(CCookedActionInfo, protocolAction.getRecord(), protocolAction))
        if planningAction:
            result._actions.append(context.getInstance(CCookedActionInfo, planningAction.getRecord(), planningAction))
    ...
    def on_actDiagnosticsAddAccomp_triggered(self):
        ...
        if currentRow>=0 :
            ...
            newRecord.setValue('diagnosisType', QtCore.QVariant(CF027Dialog.dfAccomp))
    ...
    def getHeadPersonName(self):
        db = QtGui.qApp.db
        r = db.getRecordEx(stmt="SELECT CONCAT_WS(' ', p.lastName, p.firstName, p.patrName) name FROM Person p INNER JOIN rbPost rbp ON p.post_id = rbp.id and rbp.code=1001")
        if r:
