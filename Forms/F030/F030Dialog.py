...
from Forms.F030.PreF030Dialog import CPreF030Dialog, CPreF030DagnosticAndActionPresets
from Forms.Utils import check_data_text_TNM
from Ui_F030 import Ui_Dialog
...
class CF030Dialog(CEventEditDialog, Ui_Dialog, CTableSummaryActionsMenuMixin):
    ...
    def prepare(self, clientId, eventTypeId, orgId, personId, eventSetDatetime, eventDatetime, includeRedDays, numDays,
        ...
        if QtGui.qApp.userHasRight(urAccessF030planner):
            ...
            presets = CPreF030DagnosticAndActionPresets(clientId, eventTypeId, plannerDate, self.personSpecialityId,
    ...
    def saveInternals(self, eventId):
        super(CF030Dialog, self).saveInternals(eventId)
    ...
    def setEventTypeId(self, eventTypeId):
        ...
        if not self.diagnosisSetDateVisible:
    ...
    #            return False
    #
    #        for table, modelName in [(self.tblFinalDiagnostics, u'заключительный')]:
    ...
    def checkDiagnosticResultEntered(self, table, row, record, endDate):
        ...
        if row == 0 and not endDate.isNull():
            ...
            return resultId or self.checkInputMessage(forceTr(u'результат', u'EventDiagnostic'), False, table, row,
                                                      table.model().getColIndex('result_id'))
        return True
    ...
    def createCsgComboBox(self):
        if QtGui.qApp.defaultKLADR()[:2] == '23' and QtGui.qApp.currentOrgInfis() not in ['07526', '07112', '07108']:  # ДККБ
            ...
            self.gridLayoutCsg.addLayout(self.CsgGridLayout, 1, 0, 1, 4)
            self.cmbCsg.setMinimumSize(QtCore.QSize(280, 32))
            self.deleteTabMes()
    ...
    def createHtgComboBox(self):
        if QtGui.qApp.defaultKLADR()[:2] == '23' and QtGui.qApp.currentOrgInfis() not in ['07526', '07112', '07108']:  # ДККБ
            ...
            self.CsgGridLayout.addLayout(self.HtgGridLayout, 2, 0, 1, 4)
    ...
    def on_actDiagnosticsAddAccomp_triggered(self):
        ...
        if currentRow >= 0:
            ...
            newRecord = self.modelFinalDiagnostics.getEmptyRecord()
            newRecord.setValue('diagnosisType', QtCore.QVariant(CF030Dialog.dfAccomp))
...
class CF030BaseDiagnosticsModel(CInDocTableModel):
    ...
    def __init__(self, parent, finishDiagnosisTypeCode, baseDiagnosisTypeCode, accompDiagnosisTypeCode,
        ...
        self.diagnosisTypeCol = CF030DiagnosisTypeCol(u'Тип', 'diagnosisType_id', 2,
...
class CF030FinalDiagnosticsModel(CF030BaseDiagnosticsModel):
    ...
    def addRecord(self, record):
        super(CF030FinalDiagnosticsModel, self).addRecord(record)
