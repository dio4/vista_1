...
# Родовое отделение
class CMaternitywardModel(CMonitoringModel):
    ...
    def loadData(self, params):
        ...
        maternitywardActionTable = self.tables.Action.alias('maternitywardAction')
        ...
        queryTable = queryTable.innerJoin(maternitywardActionTable, [maternitywardActionTable['event_id'].eq(self.tables.Event['id']),
                                                              maternitywardActionTable['actionType_id'].inlist(getActionTypeIdListByFlatCode('maternityward%')),
                                                              maternitywardActionTable['deleted'].eq(0),
                                                              db.joinOr([maternitywardActionTable['begDate'].le(self.tables.Action['endDate']),
                                                                         self.tables.Action['endDate'].isNull()]),
                                                              db.joinOr([maternitywardActionTable['endDate'].ge(self.tables.Action['begDate']),
                                                                         maternitywardActionTable['endDate'].isNull()])
                                                              ])
        cols.append(getDataOrgStructureName([u'Отделение%'], fieldAlias = 'currentCommonOSName', actionTableAlias = maternitywardActionTable.aliasName))
        cols.append(getAPOSNameField(maternitywardActionTable, u'Переведен из%', fieldAlias='nameFromOS'))
        cond = [self.tables.Action['actionType_id'].inlist(getActionTypeIdListByFlatCode('maternityward%')),
...
#                self.tables.Action['status'].ne(2)
        ...
        if endDateTime:
            ...
            cond.append(maternitywardActionTable['status'].ne(2))
            cond.append(maternitywardActionTable['endDate'].isNull())
        ...
        maternitywardRecords = db.getRecordList(queryTable, cols, cond, u'Client.lastName, Client.firstName, Client.patrName')
        for record in maternitywardRecords:
            item = self.getItemFromRecord(record)
            item.update({'maternitywardBegDate' : forceDateTime(record.value('begDate')).toString('dd.MM.yyyy hh:mm')})
